...
if (Config.WORKTYPE == 'private') {
    ...
    Julie.addCommand({pattern: 'owner', fromMe: true, desc: 'shows the detail of bot owner'}, (async (message, match) => {
        ...
        if (Config.OWNER == 'default') {
            await message.client.sendMessage(message.jid,'*sushilan2.0 by A4edits*' , MessageType.text);
        }
        else {
    ...
    Julie.addCommand({pattern: 'owner', fromMe: false, desc: 'shows the detail of bot owner'}, (async (message, match) => {
        ...
        if (Config.OWNER == 'default') {
            await message.client.sendMessage(message.jid,'*sushilan2.0 by Farhan_Dqz 💌 A4edits*' , MessageType.text);
        }
        else {
            await message.client.sendMessage(message.jid,Config.OWNER + '\n\n--------------------', MessageType.text);
        }
